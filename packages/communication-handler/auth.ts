...
type Response = {
  ping: () => Output<string>;
  loginWithEmailAndPassword: (input: LoginWithEmailAndPassword) => Output<any>;
}
...
export const createAuthService = ({ apiKey, baseUrl, serviceName }: Param): Response => {
  ...
  return {
    ...
    loginWithEmailAndPassword: async (input: LoginWithEmailAndPassword) => await post('/internal/login-email', input)
  }
}